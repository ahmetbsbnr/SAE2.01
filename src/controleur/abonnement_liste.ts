import { UnAbonnement, LesAbonnements, TAbonnements } from "../modele/data_abonnement.js";
import { LesAdherents } from "../modele/data_adherent.js";
import { LesCsps } from "../modele/data_csp.js";
import { LesThemes, LesThemesByAbonnement, TThemes } from "../modele/data_theme.js";

// Structure pour le formulaire de liste des abonnements
type TAbonnementListeForm = {
    divTitre: HTMLElement, 
    btnAjouter: HTMLInputElement, 
    btnAfficher: HTMLInputElement,
...
class VueAbonnementListe {
    ...
    async init(form: TAbonnementListeForm): Promise<void> {
        this._form = form;
        ...
        const data: TAbonnements = await lesAbonnements.all();
        
        // Pré-charger tous les thèmes disponibles (asynchrone)
        const allThemes: TThemes = await lesThemes.all();
        ...
        for (let num in data) {
            const unAbonnement: UnAbonnement = data[num];
            ...
            // Récupérer le nombre de thèmes (maintenant asynchrone)
        ...
        this.form.btnAfficher.onclick = async () => {
            if (this._idSelect) await this.afficherAbonnementClick(this._idSelect);
        };
        this.form.btnModifier.onclick = async () => {
            if (this._idSelect) await this.modifierAbonnementClick(this._idSelect);
        };
        this.form.btnSupprimer.onclick = async () => {
            if (this._idSelect) await this.supprimerAbonnementClick(this._idSelect);
        };
    }
    ...
    afficherAbonnementClick(num: string): void {
        location.href = "abonnement_edit.html?affi&" + encodeURIComponent(num);
    }
    
    modifierAbonnementClick(num: string): void {
        location.href = "abonnement_edit.html?modif&" + encodeURIComponent(num);
    }
    
    async supprimerAbonnementClick(num: string): Promise<void> {
